...
- **`terraform/`** — one stack per environment (`dev`, `stage`, `prod`, `mercor`, plus `jenkins`/`ansible` support stacks), a shared `modules/` for reusable resources (`do_droplet`, `do_network`, and the Kubernetes-cluster pieces `infra`/`registry`/`ingress`), and `shared/` for root-level config that's identical across stacks but can't itself be a Terraform module (provider blocks, mainly).
...
- **`-c`/`--command`** — the method to call on the environment's Runner. What's available depends on the environment's family (`Runners::Cluster` for `stage`/`prod`, `Runners::Droplet` for `dev`/`mercor`) — see `bin/lib/runners/` for the full set (`apply`, `destroy`, `run`, `database`, and `stage`/`prod`-specific steps like `infra`/`registry`/`ingress`/`kube`).
